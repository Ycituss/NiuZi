    ...
    companion object {
        suspend fun get(group: Group, member: Member) {
            ...
            if (member.pointsState() == -1) {
                group.sendLang("Status.Status-1", member.nameCardOrNick, member.id, niuzi.name, niuzi.sex.toChinese(), niuzi.length)
            } else if (member.pointsState() == 3) {
                group.sendLang("Status.Status3", member.nameCardOrNick, member.id, niuzi.name, niuzi.sex.toChinese(), niuzi.length)
            } else {
